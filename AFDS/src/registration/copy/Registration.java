...
public class Registration extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for (String currState: states)
        	out.println( "<option value='"+ currState + "'>" + currState + "</option>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String passwordConfirm = (String) request.getParameter("passwordconfirm").trim();
		...
		long creditCardNum = Long.parseLong((String) request.getParameter("creditcardnumber").trim());
		...
		if (username.length() == 0 || username == null || password.length() == 0 || password== null || passwordConfirm.length() == 0 || passwordConfirm == null){
			error = true;
		...
		if (userList.contains(username)){
			error = true;
			errorList.add("Please choose other username, this username already exists");
		}
		...
		if (!error) {
			...
			newCC.setCreditCardID(01); // auto generate credit card id based on index
